...
export default withIronSessionApiRoute(
  getAddedCheckingAccountHandler,
  sessionOptions
);

async function getAddedCheckingAccountHandler(req, res) {
  await connectToDB();
  ...
  try {
    ...
    if (loggedInUser.items && loggedInUser.items.length > 0) {
      const justAddedCheckingAccount =
        loggedInUser.items[loggedInUser.items.length - 1]?.checkingAccounts;

      return res.status(200).json(justAddedCheckingAccount);
    } else {
